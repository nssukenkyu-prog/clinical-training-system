...
export default function StudentEntry() {
    ...
    const handleLogin = async (e) => {
        ...
        try {
            const userCred = await signInWithEmailAndPassword(auth, shadowEmail, password);
            const user = userCred.user;

            // Session Setup (Optional but good for display)
            // We can fetch the doc to get the name, but strictly not required for auth.
            // Let's do a quick fetch to get the name for session storage if possible, 
            // but don't block login if it fails (though rules should allow it now).
            sessionStorage.setItem('clinical_student_id', user.uid); // Use auth_user_id as primary identifier
